        ...
        if "week" not in data or "row" not in data:
            return (
                jsonify(
                    {
                        "status": "error",
                        "message": "必須パラメータ(week, row)が不足しています",
        ...
        for column in SOUL_DIAGNOSIS_COLUMNS:
            if column in data and data[column]:
                try:
                    ...
                    logger.info(
                        f"セル更新: シート={sheet_name}, 行={row}, 列={column}, 値={data[column][:20]}..."
                    )
                except Exception as e:
                    ...
                    return (
                        jsonify(
                            {
                                "status": "error",
                                "message": f"セル更新中にエラーが発生しました: {str(e)}",
                                "error_code": "SPREADSHEET_ERROR",
                            }
                        ),
                        500,
                    )
...
# ---------------------------
SLACK_WEBHOOK_URL = (
    "https://hooks.slack.com/services/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"
)
